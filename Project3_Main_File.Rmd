...
### Plots
Next, let's investigate BMI versus whether or not a person has diabetes. A higher BMI can indicate high body fat, which can lead to health problems such as diabetes. 

```{r}

ggplot(markdown_data, aes(x=Diabetes_binary, y=BMI)) + 
  geom_point() +
  xlab("Diabetes") +
...
A classification tree is used when our goal is to classify group membership, in this case, classify whether or not an individual has diabetes. Like any tree based method, a classifciation tree will split the predictor space into mny regions and have different predictions for each region. The tree is split by minimizing the Gini Coefficient, a number that repersents how well each node classifies. 
